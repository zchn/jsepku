...
CResult::CResult()
{
	...
	a=m_dictBigram.Load("data\\BigramDict.dct");
...
bool CResult::ParagraphProcessing(const char *sParagraph, char *sResult)
{
	...
	{//Find a whole sentence which separated by ! . \n \r
		...
		if(sChar[0] < 0)
		{//双字节字符
			unsigned char s1 = sChar[0], s2 = sParagraph[nPosIndex++];
			//此两行字符 不构成全角字符 不是合法内容
			if (s1 == 128 || s1 == 255)
			{
				strcat(sSentence, " ");
				if (s2 > 127)
			...
			//改进的字符处理
			else if (s2 > 127)//标准GB2312字符 按照双字符处理
				sChar[1] = s2;
			else//扩展部分 sChar[0]在129-254 sChar[1]在64-126 合法
			{
				if ((s2 < 64) || (s2 >126)) //非法字符段
				{
					strcat(sSentence, " ");
					sChar[0] = s2;
				}
...
*/
		//找到句子分割符
		if(CC_Find(SEPERATOR_C_SENTENCE, sChar)
			|| strstr(SEPERATOR_E_SENTENCE,sChar)
			|| strstr(SEPERATOR_LINK,sChar))
...
//				if( !strstr(SEPERATOR_C_SUB_SENTENCE,sChar) && !strstr(SEPERATOR_E_SUB_SENTENCE,sChar))
			...
			strcpy(sSentence, SENTENCE_BEGIN);
		}
